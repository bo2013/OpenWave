  ...
  return (
    <div className="d-flex" style={{ flex: 1 }}>
      <img
        ...
        src={info?.image ?? "/images/not-playing.png"}
      />
...
function Player({ info, audioRef, setVolume }: PlayerProps) {
  ...
  function changeCurrentTime(
    e: SyntheticEvent<HTMLAudioElement> | ChangeEvent<HTMLInputElement>,
  ) {
    let time: number;
  ...
  function seekCurrentTime(amount: number) {
    ...
    const time = Math.max(0, Math.min(audio.currentTime + amount, duration));
  ...
  return (
    ...
    >
      <div className="flex-lg-grow-1 d-flex">
        ...
        <button className="btn btn-light rounded-circle" onClick={togglePlay}>
...
function ExtraControls({ audioRef, volume, setVolume }: ExtraControlsProps) {
  ...
  return (
    <div className="flex-grow-2 d-flex flex-row-reverse" style={{ flex: 1 }}>
      <input
        type="range"
        min={0}
        max={muted ? 0 : 1}
...
export function MediaPlayer() {
  ...
  return (
    <div className="bg-dark text-light d-flex">
      ...
      <Player info={musicInfo} audioRef={audioRef} setVolume={setVolume} />
